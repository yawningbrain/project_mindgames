...
from config import SRATE  # noqa: E402
...
def collect_samples(inlet: StreamInlet, duration_sec: int, sample_rate: int) -> tuple:
    ...
    for i in range(num_samples_expected):
        ...
        if (i + 1) % progress_interval == 0:
            ...
            msg = f"Progress: {progress:.0f}% ({i + 1}/{num_samples_expected} samples, "
            msg += f"{elapsed:.1f}s)"
            print(msg, end="\r")
...
def create_json_structure(
    ...
    for i, (sample_data, lsl_timestamp) in enumerate(samples):
        ...
        sample_obj = {"time_sec": round(relative_time, 3), "lsl_timestamp": round(lsl_timestamp, 6)}
    ...
    json_data = {"metadata": metadata, "samples": sample_objects}
...
def main():
    ...
    try:
        ...
        json_data = create_json_structure(
            samples, start_lsl_time, ch_names, SRATE, apply_conversion=APPLY_CONVERSION
        )
