...
class EEGAnalyzer:
    ...
    def print_statistics(self, stats_dict: Dict[str, Any]):
        ...
        print("=" * 90)
        header = f"{'Channel':<8} {'Mean':>10} {'Std':>10} {'Min':>10}"
        header += f" {'Max':>10} {'Median':>10} {'Range':>10}"
        print(header)
        print("-" * 90)
        ...
        for ch in self.channel_names:
            s = stats_dict[ch]
            print(
                f"{ch:<8} {s['mean']:>10.2f} {s['std']:>10.2f} {s['min']:>10.2f} "
    ...
    def plot_time_series(self, output_path: Path):
        ...
        fig.suptitle(
            f'EEG Time Series - {self.metadata["device"]}\n'
        ...
        for i, ch in enumerate(self.channel_names):
            ...
            ax.fill_between(
                self.timestamps, mean - std, mean + std, alpha=0.2, color="red", label="±1 SD"
            )
    ...
    def plot_frequency_spectrum(self, output_path: Path):
        ...
        fig, axes = plt.subplots(7, 2, figsize=(15, 14))
        fig.suptitle(
            f'Power Spectral Density - {self.metadata["device"]}\n'
    ...
    def plot_channel_heatmap(self, output_path: Path):
        ...
        data_normalized = (self.data_array - self.data_array.mean(axis=0)) / self.data_array.std(
            axis=0
        )
        ...
        ax.set_title(
            f"EEG Channel Activity Heatmap (Normalized)\n"
    ...
    def plot_band_power(self, output_path: Path):
        ...
        ax.set_title(
            f'Frequency Band Power by Channel\n{self.metadata["device"]}',
            fontsize=14,
            fontweight="bold",
        )
        ax.set_xticks(x)
    ...
    def plot_statistical_summary(self, stats_dict: Dict[str, Any], output_path: Path):
        ...
        fig.suptitle(
            f'Statistical Summary - {self.metadata["device"]}\n'
