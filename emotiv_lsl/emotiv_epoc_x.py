...
from config import DEVICE_MANUFACTURER, SRATE, STREAM_NAME
...
class EmotivEpocX(EmotivBase):
    ...
    def get_crypto_key(self) -> bytearray:
        ...
        try:
            ...
            self.logger.debug("Generating crypto key from serial number")
    ...
    def get_stream_info(self) -> StreamInfo:
        ...
        self.logger.info(
            f"Creating LSL stream: {STREAM_NAME} " f"({n_channels} channels @ {SRATE}Hz)"
        )
    ...
    def decode_data(self, data: bytes) -> list[float]:
        ...
        try:
            # XOR decryption step
            xor_data = [el ^ 0x55 for el in data]

            # AES decryption
            decrypted = self.cipher.decrypt(bytearray(xor_data))

            # Extract channel values
            packet_str = ""
            for i in range(2, 16, 2):
                packet_str += (
                    str(self._convert_value(str(decrypted[i]), str(decrypted[i + 1])))
                    + self.delimiter
                )

            for i in range(18, len(decrypted), 2):
                packet_str += (
                    str(self._convert_value(str(decrypted[i]), str(decrypted[i + 1])))
                    + self.delimiter
                )
            ...
            # Parse values
            packet_str = packet_str[: -len(self.delimiter)]
            packet_values = packet_str.split(self.delimiter)
            packet_data = [float(i) for i in packet_values]
            ...
            # Final order: ['AF3', 'F7', 'F3', 'FC5', 'T7', 'P7', 'O1', 'O2',
            # 'P8', 'T8', 'FC6', 'F4', 'F8', 'AF4']
            return packet_data
